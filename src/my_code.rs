...
// //handle get request for all user:
...
//     match (get_user_request_body(&request), get_id(&request).parse::<i32>, Client::connect(DB_URL, NoTls)) {
//         (Ok(user), Ok(id), Ok(mut client)) => {
...
// //handle delete request to delete user record:
...
//             (OK_RESPONSE.to_string(), "User record deleted".to_string())
//         }
//         _ => (INTERNAL_SERVER_ERROR.to_string(), "Error".to_string())
//     }
// }
